...
"[project]/hooks/useRTM.ts [app-client] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
"use client";
;
function useRTM({ appId, userId, token, channel, onMessage }) {
    _s();
    const [isConnected, setIsConnected] = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useState"])(false);
    const clientRef = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useRef"])(null);
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useEffect"])({
        "useRTM.useEffect": ()=>{
            if (("TURBOPACK compile-time value", "object") === "undefined" || !appId || !userId || !token || !channel || !appId.trim() || !userId.trim() || !token.trim() || !channel.trim()) {
                return;
            }
            let isMounted = true;
            const init = {
                "useRTM.useEffect.init": async ()=>{
                    try {
                        // Dynamically import agora-rtm-sdk only on the client
                        ...
                        // agora-rtm-sdk exports RTM class - use default export or named export
                        const RTM = AgoraRTM.default?.RTM || AgoraRTM.RTM || AgoraRTM.default;
                        if (!RTM || typeof RTM !== "function") {
                            throw new Error("Failed to get RTM class from agora-rtm-sdk");
                        }
                        const client = new RTM(appId, userId);
                        clientRef.current = client;
                        // Debug: Log RTM login parameters
                        console.log("[RTM] Login attempt:", {
                            appId,
                            userId,
                        ...
                        if (!isMounted) {
                            await client.logout().catch({
                                "useRTM.useEffect.init": ()=>{}
                            }["useRTM.useEffect.init"]);
                            return;
                        }
                        // Listen for peer-to-peer messages (as per ConversationalAIAPI toolkit)
                        client.on("MessageFromPeer", {
                            "useRTM.useEffect.init": (message, peerId)=>{
                                if (isMounted && onMessage) {
                                    try {
                                        // Parse the message payload (format from toolkit)
                                        const payload = JSON.parse(message.text);
                                        if (payload.message) {
                                            onMessage(payload.message, peerId);
                                        }
                                    } catch (e) {
                                        // Fallback: try to get text directly
                                        onMessage(message.text || message.message || "", peerId);
                                    }
                                }
                            }
                        }["useRTM.useEffect.init"]);
    ...
    const sendMessage = async (text, agentUserId = "1000")=>{
        if (!clientRef.current || !isConnected) {
            throw new Error("RTM client not connected");
        }
        try {
            // Use the same format as the ConversationalAIAPI toolkit
            const payload = {
                priority: 1,
                interruptable: true,
                message: text
            ...
            // Publish to agent user (peer-to-peer, not channel)
            await clientRef.current.publish(agentUserId, payloadStr, options);
        } catch (error) {
...
_s(useRTM, "D7Y47KTzLGEnWh6b3t8pXV9LGw8=");
if (typeof globalThis.$RefreshHelpers$ === 'object' && globalThis.$RefreshHelpers !== null) {
    __turbopack_context__.k.registerExports(__turbopack_context__.m, globalThis.$RefreshHelpers$);
}
}),
...
var __TURBOPACK__imported__module__$5b$project$5d2f$hooks$2f$useRTM$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/hooks/useRTM.ts [app-client] (ecmascript)");
;
var _s = __turbopack_context__.k.signature();
"use client";
;
...
function ChatInterface({ product, onClose }) {
    ...
    const [productContext, setProductContext] = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useState"])(null);
    ...
    // Initialize Agora RTC connection - always call hook but only connect when config is ready
    ...
    // Initialize RTM for text messaging
    const rtm = (0, __TURBOPACK__imported__module__$5b$project$5d2f$hooks$2f$useRTM$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useRTM"])(agoraConfig ? {
        appId: agoraConfig.appId,
        userId: agoraConfig.rtmUserId,
        token: agoraConfig.token,
        channel: agoraConfig.channel,
        onMessage: ({
            "ChatInterface.useRTM[rtm]": (message, senderId)=>{
                // Only show messages from the agent (UID 1000 or agent ID)
                if (senderId === "1000" || senderId === agentId) {
                    setMessages({
                        "ChatInterface.useRTM[rtm]": (prev)=>[
                                ...prev,
                                {
                                    role: "assistant",
                                    content: message
        ...
        "ChatInterface.useEffect": ()=>{
            const initializeAgora = {
                "ChatInterface.useEffect.initializeAgora": async ()=>{
                    ...
                    try {
                        // Get product context
                        const contextRes = await fetch("/api/agora/product-context");
                        const contextData = await contextRes.json();
                        setProductContext(contextData.context);
                        ...
                        // Store agent_id for chat messages
            ...
            initializeAgora();
        // Note: cleanup is handled by useAgora hook itself
        }
    }["ChatInterface.useEffect"], [
        product.id
    ]);
    const sendMessage = async ()=>{
        if (!input.trim() || isLoading || !rtm.isConnected) return;
        ...
        setIsLoading(true);
        try {
            // Send message via RTM to agent (using toolkit format)
            await rtm.sendMessage(userMessage, "1000"); // Agent RTM user ID is "1000"
            // Response will come through RTM onMessage callback
            setIsLoading(false);
        } catch (error) {
...
_s(ChatInterface, "cPRqlmWMMGE6vTVfs5vwbJakrLc=", false, function() {
    return [
        ...
        __TURBOPACK__imported__module__$5b$project$5d2f$hooks$2f$useRTM$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useRTM"]
    ];
});
_c = ChatInterface;
var _c;
...
//# sourceMappingURL=_0b0396fc._.js.map