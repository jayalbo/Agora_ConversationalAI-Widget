...
"[project]/lib/agoraClient.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
function assertAgoraEnv() {
    if (!APP_ID || !APP_CERTIFICATE || !API_KEY || !API_SECRET) {
        throw new Error("Missing Agora credentials. Please set AGORA_APP_ID, AGORA_APP_CERTIFICATE, AGORA_API_KEY, AGORA_API_SECRET.");
    }
}
function generateToken(channelName, uid) {
    ...
    // Use buildTokenWithAccount for string UIDs, buildTokenWithUid for numeric UIDs
    const token = typeof uid === "string" ? RtcTokenBuilder.buildTokenWithAccount(APP_ID, APP_CERTIFICATE, channelName, uid, role, privilegeExpiredTs) : RtcTokenBuilder.buildTokenWithUid(APP_ID, APP_CERTIFICATE, channelName, uid, role, privilegeExpiredTs);
    return token;
}
async function startAgent(channelName, userId, context, token) {
    assertAgoraEnv();
    try {
        ...
        // Handle 409 Conflict - agent already running, return existing agent info
        if (status === 409 && data?.agent_id) {
            console.log("[startAgent] Agent already running, returning existing agent:", data.agent_id);
            return {
        ...
        const message = data?.detail || data?.message || err?.message || "Unknown error starting Agora agent";
        const details = {
            status,
            data
        };
        throw new Error(`${message}${status ? ` (status ${status})` : ""}::${JSON.stringify(details)}`);
    }
}
async function processChatMessage(message, context, productId) {
    try {
        ...
        if (isOffTopic) {
            return {
                response: "I'm here to help you with this product. Would you like to know about its features, reviews, or specifications?",
                isOffTopic: true
            };
        }
        const response = await __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$axios$2f$lib$2f$axios$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["default"].post("https://api.agora.io/v1/conversational-ai/chat", {
            message: message,
            context: context,
        ...
        return {
            response: "I'm having trouble processing that right now. Could you rephrase your question about the product?",
            isOffTopic: false
        };
    }
}
...
var __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$agoraClient$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/lib/agoraClient.ts [app-route] (ecmascript)");
...
7. Keep responses focused and product-related`;
        const result = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$agoraClient$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["startAgent"])(channel, userId, context, token);
        console.log("Response:", JSON.stringify(result, null, 2));
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json(result);
    } catch (error) {
        const err = error;
        const message = err?.message?.split("::")?.[0] || err?.response?.data?.detail || "Failed to start AI agent";
        const detail = err?.response?.data || err?.message;
        console.error("[start-agent] error:", JSON.stringify(detail, null, 2));
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
            error: message,
            detail
        }, {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__5dcce699._.js.map